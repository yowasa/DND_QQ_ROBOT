...
def selectRecord(id):
    ...
    recordurl = baseurl + "/player_records/" + str(
        id) + "/" + urltime + "/1262304000000?limit=2&mode=16.12.9.15.11.8&descending=true&tag=" + count
...
def selectTriRecord(id):
    ...
    if isinstance(data, urllib.error.URLError):
        return -1
    count = str(json.loads(data)["count"])
    recordurl = tribaseurl + "/player_records/" + str(
        id) + "/" + urltime + "/1262304000000?limit=2&mode=22.24.26.21.23.25&descending=true&tag=" + count
    record = getURL(recordurl)
    if isinstance(record, urllib.error.URLError):
        return -1
    return record
