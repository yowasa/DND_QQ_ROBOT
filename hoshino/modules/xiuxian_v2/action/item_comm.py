...
async def _(bot, ev: CQEvent):
    ...
    if not item_info:
        await bot.finish(ev, f"未找到名称为{msg}的道具", at_sender=True)
    user = UserDao.get_ev_user(bot, ev)
    ItemDao.add_item_ignore_limit(user.uid, item_info)
...
async def _(bot, ev: CQEvent):
    ...
    num = int(msg)
    UserModelDao.add_user_counter(uid, UserModel.LINGSHI, num)
