...
async def youli(bot, ev: CQEvent):
    ...
    all_li = EVENT_MAP.get(user.local)
    if not all_li:
    ...
    if user.gongfa3 == '飞龙探云手':
        if random.randint(1, 10) == 1:
            get_lingshi = user.grade * 10
            UserModelDao.add_user_counter(user.uid, UserModel.LINGSHI, get_lingshi)
            await bot.send(ev, f"你发动了飞龙探云手，获取了{get_lingshi}灵石", at_sender=True)
    id = UserModelDao.get_user_counter(user.uid, UserModel.YOULI_DAQIAN)
    if id:
        if YOULI_DAQIAN_MAP[str(id)]['name'] == user.map:
            UserModelDao.add_user_counter(user.uid, UserModel.YOULI_DAQIAN_COUNT)
