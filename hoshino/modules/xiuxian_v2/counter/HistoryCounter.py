...
DUEL_DB_PATH = os.path.expanduser(BASE_DB_PATH + 'xiuxian_his_v2.db')
...
class UserHistory(Enum):
    ...
    MAX_HP = [4, "死亡时最高的血量"]
    MAX_MP = [5, "死亡时最高的蓝量"]
    MAX_ATK1 = [6, "死亡时最高的物理攻击"]
    MAX_ATK2 = [7, "死亡时最高的术法攻击"]
    MAX_DEFEND1 = [12, "死亡时最高的物理防御"]
    MAX_DEFEND2 = [13, "死亡时最高的术法防御"]
    RESTART_ABILITY= [14, "死亡时随机留存一个功法"]
...
class HistoryCounter:
        ...
        try:
            self._connect().execute('''CREATE TABLE IF NOT EXISTS USER_INFO
                          (UID           INT    NOT NULL,
                           ...
                           PRIMARY KEY(UID,BUFF_TYPE));''')
        except:
            raise Exception('创建用户状态表发生错误')

    # 存储用户信息
    def _save_user_info(self, uid, user_state: UserHistory, flag):
        with self._connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO USER_INFO (UID, BUFF_TYPE, BUFF_INFO) VALUES (?, ?, ?, ?)",
                (uid, user_state.value[0], flag),
            )

    # 获取用户信息
    def _get_user_info(self, uid, user_state: UserHistory):
        try:
    ...
    # 获取用户信息
    def _query_user_info(self, uid):
        try:
            r = self._connect().execute(
                "SELECT BUFF_TYPE,BUFF_INFO FROM USER_INFO WHERE UID=? AND BUFF_INFO>0",
                (uid),
            ).fetchall()
            if not r:
                return []
