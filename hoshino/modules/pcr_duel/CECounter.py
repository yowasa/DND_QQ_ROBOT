...
class CECounter:
    ...
    def _get_equip_num(self, gid, uid, eid):
        try:
            r = self._connect().execute("SELECT NUM FROM EQUIPMENT WHERE GID=? AND UID=? AND EID=?", (gid, uid, eid)).fetchone()
            return 0 if r is None else r[0]
        except:
    ...
    def _get_dress_info(self, gid, uid, cid, typeid):
        try:
            r = self._connect().execute("SELECT EID FROM DRESSEQUIP WHERE GID=? AND UID=? AND CID=? AND TYPEID=? AND EID>0", (gid, uid, cid, typeid)).fetchone()
            return 0 if r is None else r[0]
        except:
    ...
    def _get_fragment_num(self, gid, uid, cid):
        try:
            r = self._connect().execute("SELECT NUM FROM FRAGMENT WHERE GID=? AND UID=? AND CID=?", (gid, uid, cid)).fetchone()
            if r is None:
               return 0
            return r[0]
        except Exception as e:
    ...
    def _get_cardstar(self, gid, uid, cid):
        try:
            r = self._connect().execute("SELECT NUM FROM CARDSTAR WHERE GID=? AND UID=? AND CID=?", (gid, uid, cid)).fetchone()
            if r is None:
               return 0
            return r[0]
        except Exception as e:
