...
def build_result(subscribe,illusts):
    ...
    logs=SubscribeSendLog.select().where((SubscribeSendLog.user_id == subscribe.user_id) & (SubscribeSendLog.user_type == subscribe.user_type) & (SubscribeSendLog.message_id in ill_ids))
    for log in logs:
